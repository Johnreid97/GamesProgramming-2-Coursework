...
void GameObject::initialise(Vertex* vertices, unsigned int numVertices, unsigned int* indices, unsigned int numIndices)
{
	IndexedModel model;
...
void GameObject::initialiseModel(const IndexedModel& model)
{
	...
	glGenBuffers(NUM_BUFFERS, vertexArrayBuffers); //generate our buffers based of our array of data/buffers
...
void GameObject::createModel(const std::string& filename)
{
	//creates the model
...
void GameObject::AttachSphereCollider(glm::vec3 pos, float radius)
{
	//creates a sphere collider around gameobject
	colliderSphere.SetPos(pos);
